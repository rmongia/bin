...
SOURCE_PATH=`cd $HOME/kido/; pwd` # convert $1 to an absolute path
find $SOURCE_PATH\
    -name "*.cpp" -or\
    -name "*.hpp" -or\
    -name "*.cxx" -or\
...
find $SOURCE_PATH -newer files.txt\
    | egrep "\.(cpp|cxx|cc|c|hpp|hxx|h)$"\
    | sort > filesmodified.txt
...
for i in `cat filesmodified.txt`; do
